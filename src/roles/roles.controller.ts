...
export class RolesController {
  constructor(private readonly privilegeService: PrivilegeService) {}
    ...
    async createRole(@Res() res:Response,@TokenDecoder() user:DecodedUser,@Body() createRoleDto:Partial<RoleDocument> ) {
        const result = await this.privilegeService.createRole(user.id,user.userType,createRoleDto);
